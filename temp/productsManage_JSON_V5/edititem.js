...
router.get('/', function(req, res, next) {
  ...
  function showinfo(sqlcmd) {     //顯示全部資料function
    connectdb.query(sqlcmd, function(err, rows) {
      ...
      if(rows.length!==0) res.json({rows:rows});
...
router.put('/',upload.single('itempic')/*上傳單檔*/, function(req, res) {
 ...
 if(imgtodb.name==='') {res.json({fail:'請輸入商品名稱'}); return;}
 if(imgtodb.price==='')  { res.json({fail:'請輸入商品價格'});  return;  }
 if(imgtodb.store==='')  { res.json({fail:'請輸入商品存量'});  return;  }
 imgtodb.photo=base64_encode(req.file.filename); //將base64str加入imgtodb物件photo欄位
//---------------------------------------
